...
class EKF():
    ...
    def __init__(self,dt,sensor_size,var_z,beacons,coeff):
        ...
        self.Ex = np.array([[dt**3/3, 0,dt**2/2, 0],[0, dt**3/3, 0,dt**2/2], [dt**2/2, 0,dt, 0], [0, dt**2/2, 0, dt]])
        self.F = np.array([[1, 0, dt, 0],[0, 1, 0, dt], [0, 0, 1, 0], [0, 0, 0, 1]])
        self.x_hat = np.array() #shape is (1,4) - to init with initial position
        self.Ez = np.eye(M=sensor_size, N=sensor_size)*var_z
        self.P = self.Ex
    ...
    def prediction(self):
        ...
        measurements = self.selective_extraction()
        h = np.zeros(self.sensor_size)
        for i in range(0,self.sensor_size):
            if (measurements[i] != 0):
                #h[i] = distance(self.x_hat[0],self.beacons[i])
                h[i] = self.space_to_value(self.x_hat[0] - self.beacons[i])

        #H is the measurement equation
        H = np.empty((0,4))
        for i in len(measurements):
            if (measurements[i] != 0):
                dh_dx, dh_dy = self.derivative(self.x_hat[0] - self.beacons[i])
            else:
                dh_dx = 0.0
                dh_dy = 0.0
    ...
    def selective_extraction(self,P_hat):
        ...
        measurements = []
        for i in indexes:
            data = self.client.pollBasestation(self.beacons.address(i))
    ...
    def position_selector(self,P_hat):
        ...
        for i in self.sensor_size:
            b_position = np.append(self.beacons[i], np.array([0,0]));
